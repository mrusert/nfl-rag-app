...
def describe_spread_result(spread_line: float, result: int, home_team: str, away_team: str) -> str:
    ...
    # The spread is from the home team's perspective
    # If spread_line is -3, home team is favored by 3
    # If spread_line is +3, away team is favored by 3
    ...
    if spread_line < 0:
        ...
        if home_covered:
            return f"{home_team} covered as {fav_spread}-point favorites."
        else:
            return f"{away_team} covered as {fav_spread}-point underdogs."
    else:
        ...
        if away_covered:
            return f"{away_team} covered as {fav_spread}-point favorites."
        else:
            return f"{home_team} covered as {fav_spread}-point underdogs."
...
def describe_rest_advantage(home_rest: int, away_rest: int, home_team: str, away_team: str) -> str:
    ...
    if diff > 2:
        return f"{home_team} had a significant rest advantage ({home_rest} days vs {away_rest} days)."
    elif diff < -2:
        return f"{away_team} had a significant rest advantage ({away_rest} days vs {home_rest} days)."
    elif diff > 0:
        return f"{home_team} had slightly more rest ({home_rest} vs {away_rest} days)."
    elif diff < 0:
        return f"{away_team} had slightly more rest ({away_rest} vs {home_rest} days)."
    else:
...
def describe_weather(weather: dict) -> str:
    ...
    if not weather or not weather.get("weather_fetched"):
        if weather and not weather.get("is_outdoor_game", True):
            return "This game was played in a dome."
        return ""
    ...
    if temp is not None:
        if feels_like is not None and abs(temp - feels_like) > 5:
            parts.append(f"{temp:.0f}°F (felt like {feels_like:.0f}°F)")
        else:
    ...
    if wind is not None and wind > 10:
        if wind_dir:
            parts.append(f"{wind:.0f} mph {wind_dir} wind")
        else:
    ...
    if snow > 0:
        parts.append(f"{snow:.1f}\" snow")
    elif precip > 0:
        parts.append(f"{precip:.2f}\" precipitation")
...
def player_season_chunk(player: dict) -> tuple[str, dict]:
    ...
    team = player.get("recent_team", player.get("team", "Unknown"))
    
    # Build the text
    lines = [
        f"{name} ({position}, {team}) - {season} Season Statistics"
    ]
    ...
    if pass_yards and pass_yards > 0:
        ...
        lines.append(
            ...
            f"{format_number(pass_tds, 0)} TDs, {format_number(ints, 0)} INTs "
            f"({format_number(completions, 0)}/{format_number(attempts, 0)}, {comp_pct:.1f}% completion)"
        )
    ...
    if rush_yards and (rush_yards > 50 or position == "RB"):
        ...
        lines.append(
            ...
            f"{format_number(rush_tds, 0)} TDs on {format_number(carries, 0)} carries "
            f"({ypc:.1f} YPC)"
        )
    ...
    if rec_yards and (rec_yards > 50 or position in ("WR", "TE")):
        ...
        lines.append(
            ...
            f"{format_number(rec_tds, 0)} TDs on {format_number(receptions, 0)} catches "
...
def player_game_chunk(player: dict, game: Optional[dict] = None) -> tuple[str, dict]:
    ...
    position = player.get("position", "Unknown")
    team = player.get("recent_team", player.get("team", "Unknown"))
    opponent = player.get("opponent_team", "Unknown")
    
    # Determine home/away
    # In weekly stats, we may need to infer this from the game data
    is_home = None
    if game:
    ...
    location_str = ""
    if is_home is True:
        location_str = f"vs {opponent} (home)"
    elif is_home is False:
        location_str = f"@ {opponent} (away)"
    else:
    ...
    if game:
        ...
        if game_type == "POST":
            week_type = "Playoffs Week"
        elif game_type == "SB":
            week_type = "Super Bowl"
    
    lines = [
        f"{name} ({position}, {team}) - {season} {week_type} {week} {location_str}"
    ]
    ...
    if game:
        ...
        if spread is not None:
            if spread < 0:
                # Home favored
                if is_home:
                    lines.append(f"{team} was favored by {abs(spread)} points")
                else:
                    lines.append(f"{team} was a {abs(spread)}-point underdog")
            else:
                # Away favored
                if is_home:
                    lines.append(f"{team} was a {spread}-point underdog")
                else:
                    lines.append(f"{team} was favored by {spread} points")
        ...
        if home_score is not None and away_score is not None:
            ...
            if team_score > opp_score:
                result_str = f"{team} won {team_score}-{opp_score}"
            elif team_score < opp_score:
                result_str = f"{team} lost {opp_score}-{team_score}"
            else:
                result_str = f"Game tied {team_score}-{opp_score}"
    ...
    if pass_yards and pass_yards > 0:
        lines.append(
            f"Passing: {format_number(pass_yards, 0)} yards, "
            f"{format_number(pass_tds, 0)} TDs, {format_number(ints, 0)} INTs "
            f"({format_number(completions, 0)}/{format_number(attempts, 0)})"
        )
...
def game_summary_chunk(game: dict) -> tuple[str, dict]:
    ...
    if game_type == "SB":
        ...
        week_str = "Wild Card Round"
    elif game_type == "DIV":
        week_str = "Divisional Round"
    elif game_type == "CON":
    ...
    if home_coach and away_coach:
        lines.append(f"Coaches: {away_coach} ({away_team}) vs {home_coach} ({home_team})")
    
    # Rest days
    home_rest = game.get("home_rest")
    away_rest = game.get("away_rest")
    rest_desc = describe_rest_advantage(home_rest, away_rest, home_team, away_team)
    if rest_desc:
    ...
    if spread is not None:
        if spread < 0:
            lines.append(f"Line: {home_team} favored by {abs(spread)} points")
        elif spread > 0:
            lines.append(f"Line: {away_team} favored by {spread} points")
        else:
            lines.append("Line: Pick'em (no spread)")
    
    if total_line is not None:
        lines.append(f"Over/Under: {total_line}")
    ...
    if home_score is not None and away_score is not None:
        ...
        if home_score > away_score:
            lines.append(f"Final: {home_team} {home_score}, {away_team} {away_score}")
            lines.append(f"{home_team} won by {home_score - away_score} points")
        elif away_score > home_score:
            lines.append(f"Final: {away_team} {away_score}, {home_team} {home_score}")
            lines.append(f"{away_team} won by {away_score - home_score} points")
        else:
            lines.append(f"Final: {away_team} {away_score}, {home_team} {home_score} (TIE)")
        ...
        if spread is not None and result is not None:
            spread_result = describe_spread_result(spread, result, home_team, away_team)
            if spread_result:
    ...
    if game.get("div_game"):
        lines.append("This was a divisional game.")
    ...
    if home_score is not None and away_score is not None:
        ...
        metadata["winner"] = home_team if home_score > away_score else (away_team if away_score > home_score else "TIE")
...
def player_bio_chunk(player: dict) -> tuple[str, dict]:
    ...
    if draft_year and draft_round and draft_pick:
        lines.append(f"Draft: {draft_year}, Round {draft_round}, Pick {draft_pick}")
    elif draft_year:
...
def team_info_chunk(team: dict) -> tuple[str, dict]:
    ...
    lines = [f"{name} ({abbr}) - Team Profile"]
    ...
    if stadium:
        lines.append(f"Stadium: {stadium}")
